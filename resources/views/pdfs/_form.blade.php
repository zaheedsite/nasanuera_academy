        ...
        <div class="bg-white shadow-2xl rounded-3xl w-full max-w-4xl p-10">
            ...
            <form action="{{ $form_action }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- Upload PDF --}}
                    <div>
                        <label for="pdf_file" class="block font-semibold text-gray-700 mb-1">Upload PDF</label>
                        <input type="file" name="pdf_file" id="pdf_file" accept="application/pdf"
                            class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-400 focus:outline-none">

                        @if(isset($pdf) && $pdf->pdf_url)
                            <p class="mt-2 text-sm text-gray-600">
                                File saat ini: <a href="{{ $pdf->pdf_url }}" target="_blank" class="text-emerald-600 underline">Lihat PDF</a>
                            </p>
                        @endif
                    </div>
                </div>
